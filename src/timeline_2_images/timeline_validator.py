...
def _validate_field_is_list(data: dict, field: str, errors: list) -> bool:
...
def _check_data_is_dict(data: dict) -> None:
...
def _check_has_data_sources(data: dict) -> None:
    """Verify data has at least one data source."""
    errors: list[str] = []
    fields = ["semanticSegments", "timelineObjects", "locations"]
    has_data = any(_validate_field_is_list(data, field, errors) for field in fields)
...
def validate_timeline_structure(json_path: str) -> dict:
    ...
    data: Any = _load_json_file(json_path)
    _check_data_is_dict(data)
    _check_has_data_sources(data)
    return data  # type: ignore


def _collect_segment_errors(segment: dict, segment_index: int) -> list[str]:
...
def validate_segment_structure(segment: dict, segment_index: int = 0) -> None:
    ...
    errors = _collect_segment_errors(segment, segment_index)
    if errors:
        raise TimelineValidationError("\n".join(errors))
